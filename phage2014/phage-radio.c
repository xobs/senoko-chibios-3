...
void radioSetChannel(uint32_t channel) {
  ...
  radio_lld_send(sizeof(buffer), buffer);
...
uint32_t radioGetAddress(void) {
  ...
  radio_select();
  buf = 0x23;
  radio_lld_send(1, &buf);
  radio_lld_receive(4, &currAddr);
  radio_unselect();
...
void radioGetStatus(uint8_t buf[11]) {
  ...
  radio_select();
  radio_lld_send(1, &byte);
  radio_lld_receive(11, buf);
  radio_unselect();
}
...
void radioStart(void)
{
  ...
  dat[10] = 0x5B;
  radio_select();
  radio_lld_send(11, dat);
  radio_lld_receive(1, dat);
  radio_unselect();
